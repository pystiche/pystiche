...
``style_loss`` and optionally a ``regularizer``. As the name implies the ``content``-
and ``style_loss`` indicate how well the stylized image matches the content and style
of the target images. A regularizer can be used to suppress artifacts in the stylized
image.
