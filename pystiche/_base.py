...
class _StateObjectStorage(OrderedDict):
    pass


class StateObject(Object):
    def __init__(self) -> None:
        self._state_obj_storage = _StateObjectStorage()

    def _is_state_obj(self, obj: Any) -> bool:
        return isinstance(obj, (torch.Tensor, nn.Module, StateObject))

    def __setattr__(self, name: str, value: Any):
        if name == "_state_obj_storage":
            if not isinstance(value, _StateObjectStorage):
                # TODO: add error message, "variable is reserved for internal usage"
                raise RuntimeError
            object.__setattr__(self, name, value)
            return

        is_initialized = "_state_obj_storage" in self.__dict__
        is_state_obj = self._is_state_obj(value)
        if is_initialized:
            if is_state_obj:
                self._state_obj_storage[name] = value
                if name in self.__dict__:
                    del self.__dict__[name]
            else:
                object.__setattr__(self, name, value)
                if name in self._state_obj_storage:
                    del self._state_obj_storage[name]
        else:
            if is_state_obj:
                msg = (
                    "Cannot assign an object of type "
                    "{torch.Tensor, nn.Module, pystiche.StateObject}"
                    "before the __init__() call."
                )
    ...
    def __getattr__(self, name: str) -> Any:
        # FIXME: is this only called when object does not have an attribute with that name?
        try:
            return self._state_obj_storage[name]
        except KeyError:
    ...
    def __dir__(self) -> Iterable[str]:
        keys = itertools.chain(
            dir(self.__class__), self.__dict__.keys(), self._state_obj_storage.keys()
        )
        return sorted(list(filter(is_valid_variable_name, keys)))

    def children(self) -> Iterator[Union[torch.Tensor, nn.Module, "StateObject"]]:
        yield from self._state_obj_storage.values()
        yield from self.extra_children()

    def extra_children(self) -> Iterator[Union[torch.Tensor, nn.Module, "StateObject"]]:
        # each state object can define this to yield extra children
        # this is useful if you have a collection of objects to which a function should be applied
        yield
        return
    ...
    def apply_(self, fn: Callable):
        for obj in self.children():
            ...
            elif isinstance(obj, (nn.Module, StateObject)):
                obj.apply(fn)
